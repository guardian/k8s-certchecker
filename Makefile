...
certchecker.linux64: main.go certs/check_cert.go certfinder/scanner.go
	GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build -o certchecker.linux64

certchecker.macos: main.go certs/check_cert.go certfinder/scanner.go
...
clean:
	rm -f certchecker certchecker.linux64 certchecker.macos